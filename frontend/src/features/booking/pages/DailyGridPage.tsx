    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <header className="bg-white border-b border-gray-200 px-4 py-3">
        <div className="max-w-5xl mx-auto flex items-center gap-2 flex-wrap">
          ...
          <h1 className="text-base font-semibold text-gray-900">Grilla del dia</h1>
          ...
          <div className="ml-auto flex items-center gap-2">
            ...
            <input
              ...
              className="rounded-lg border border-gray-300 bg-white px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500 focus:border-brand-500"
            />
          </div>
        </div>
      </header>
            ...
            <table className="min-w-full border-collapse text-sm">
              <thead>
                <tr className="border-b border-gray-100 bg-gray-50">
                  ...
                  <th
                    scope="col"
                    className="sticky left-0 z-10 bg-gray-50 px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wide border-r border-gray-100 min-w-[80px]"
                  >
                    Horario
                  </th>
                  {courts.map((court) => (
                    <th
                      key={court.id}
                      scope="col"
                      className="px-4 py-3 text-center text-xs font-semibold text-gray-700 uppercase tracking-wide min-w-[130px]"
                    >
                      <span className="block">{court.name}</span>
                      <span className="block font-normal text-gray-400 normal-case">
                        {court.type}
                      </span>
                    </th>
                  ))}
              ...
              <tbody className="divide-y divide-gray-100">
                {uniqueTimes.map((startDt) => (
                  <tr key={startDt} className="hover:bg-gray-50/50 transition-colors">
                    {/* Celda de horario — sticky */}
                    <td
                      className="sticky left-0 z-10 bg-white px-4 py-2.5 text-xs font-semibold text-gray-600 border-r border-gray-100 whitespace-nowrap"
                    >
